...
class SOSRequest(BaseModel):
    ...
    units: int = Field(..., ge=1, description="Number of units needed")
    urgency: str = Field(..., description="Urgency level")
    hospital: str = Field(..., description="Hospital or facility name")
    city: str = Field(..., description="City or location")
    contact_name: str = Field(..., description="Contact person name")
    contact_phone: str = Field(..., description="Contact mobile number")
